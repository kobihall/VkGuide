...
The data-upload `createImage(void* data, ...)` overload originally **hardcoded its staging-buffer size as `width * height * depth * 4`** — correct only for RGBA8. *Update 2026-09-13: fixed.* It now sizes the upload with a per-format `bytes_per_pixel()` lookup (RGBA8/BGRA8 = 4, RGBA16F = 8, RGBA32F = 16, aborts otherwise), pulled forward from `docs/plans/completed/scene-and-asset-management.md` §2.4 for the CPU raytracer's `R32G32B32A32_SFLOAT` output (`docs/plans/completed/raytracing-in-a-weekend.md` §9.6).
...
- *Update 2026-09-14* (`docs/plans/completed/scene-and-asset-management.md`): the scene is now composed at runtime: `VulkanEngine::m_models` (replacing `m_loadedScenes`) holds any number of imported glTF files, `importGltf()/removeGltf()` change it and bump `m_sceneRevision`, and `openScene()/saveScene()` move models + environment map + spheres together as one `.gltf` scene file (`src/scene_io.h`) via native dialogs (`src/file_dialog.mm`); the tutorial's hardcoded Suzanne/cubes (`m_testMeshes`, `m_loadedNodes`) are gone; `m_raytraceMeshData` holds object-space mesh data rebuilt only on model changes; `m_environmentMap` (rgba16f) is loaded by `loadEnvironmentMap()`; `TransformGizmo` (`src/vk_gizmo.h`, ImGuizmo 1.10 at `../CPPLibraries/ImGuizmo`) is the shared viewport gizmo; `IoResult` (`vk_types.h`) is the recoverable-error type for user-facing file I/O, the one deliberate exception to the hard-abort convention below.
