...
5. **`docs/plans/completed/compute-pipeline-raytracing.md`** *(implemented 2026-09-15, CPU backend retired the same day; read its §9, and §9.7 for the scene cameras and restart-on-change added after the test pass)* — A GPU path tracer that **replaced** (2)'s CPU raytracer behind the same panel, Render button and output window. Moves the scene to a plain-data model first, keeps the raster viewport as the live preview and renders a snapshot of its camera progressively; wavefront execution (compacted queues, indirect dispatch), environment-map miss lighting, Russian roulette and an adaptive-sampling hook. Depends on (2), (3) and (4).
