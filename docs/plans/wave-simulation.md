...
4. **Whether wave-simulation parameters (wave speed, grid resolution) should be saveable**, extending `docs/plans/completed/scene-and-asset-management.md`'s save/load mechanism the way `docs/plans/simulation-domain.md` §8 already flagged for boundary-condition assignments — same open question, not resolved here either.
