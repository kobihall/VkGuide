...
Every loaded mesh instance (walking `VulkanEngine::m_loadedScenes` → each `LoadedGLTF::topNodes` → `MeshNode`s, using each one's `worldTransform` — the exact same tree-walk pattern `docs/plans/completed/raytracing-in-a-weekend.md`'s `buildRaytraceScene()`-equivalent already established for a different purpose) has its triangles (`MeshAsset::cpuVertices`/`cpuIndices`, transformed to world space by the node's `worldTransform`) tested against the plane: classify each triangle vertex by signed distance to the plane, and where a triangle's vertices don't all share the same sign, compute the edge-crossing segment. Collected per source `MeshNode` (matching the per-node granularity `docs/plans/completed/raytracing-in-a-weekend.md`'s object browser already uses), these segments are projected into the plane's local 2D coordinate space (using its basis vectors) to become that mesh instance's **boundary region**.
...
- **CPU mesh data source**: `MeshAsset::cpuVertices`/`cpuIndices` (`docs/plans/completed/raytracing-in-a-weekend.md` §2.3) — this is the first real consumer of that previously-unused scaffolding.
...
- **List-driven object browser + per-item settings**: `RaytraceSceneEditor`'s unified sphere/glTF-node browser (`docs/plans/completed/raytracing-in-a-weekend.md` §2.1/§2.4) is the shape the boundary-condition-assignment panel should follow.
