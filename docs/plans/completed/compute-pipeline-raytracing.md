...
### 9.7 Follow-up after the user's test pass (2026-09-15)

Every test in §9.4 passed, with one note: ImGui as set up here does not dock windows into the main viewport, only onto other windows, so the "Match viewport" A/B in test 4 was done side by side rather than docked. The user then asked for the retirement and four additions; where this section disagrees with §§1–9.6, this section is right.

**Retired (§2.14).** `rt_job`, `rt_random`, `rt_hittable`, `rt_material`, `rt_types` and `RaytraceScene` are deleted; `RaytraceRenderer` has no Backend combo and `GpuPathTracer` is the only backend. `docs/plans/completed/raytracing-in-a-weekend.md` §10 records it.

**Gizmo lag fixed.** The free camera's position was integrated inside `updateScene()`, which runs in `draw()` *after* the gizmo (and every panel) had already been built with the previous frame's pose, so the gizmo trailed the raster view by one frame of movement and looked detached while flying. `VulkanEngine::updateViewportCamera()` now runs at the top of the loop, before `ImGuizmo::BeginFrame()`, so the gizmo, the overlays, the panels and the raster pass all see one pose.

**Scene cameras.** `SceneCamera` (`rt_scene_types.h`: id, name, position, quaternion orientation, vertical fov, aperture, focus distance, exposure) is a scene object like a sphere: listed first in the Scene panel's Objects list, selectable, with position / rotation (pitch, yaw, roll) / fov / lens / exposure controls, "Edit Transform" (gizmo, translate + rotate in local mode) and "First-Person Edit". The free raster camera is *not* a scene camera; `Add Camera` creates one at the viewport's pose. The lens and exposure moved here from `RenderSettings` (§2.6 is superseded): they describe a camera, not a render. `cameraSnapshot(const SceneCamera&)` replaces `captureCameraSnapshot()`; the render panel has a Camera combo (`RaytraceRenderer::renderCameraId()`), and a scene with no camera cannot render. Every camera is drawn in the viewport as a wire frustum (`drawCameraOverlays()`, imgui's background draw list, aspect from the render settings), the render camera in yellow, with a triangle marking up so roll is readable.

**First-person edit.** `beginFirstPersonEdit(id)` puts the free camera at the scene camera's pose (yaw and pitch from its forward vector; the free camera has no roll) and, every frame after, `updateViewportCamera()` writes the viewport's pose into the scene camera through `RaytraceSceneEditor::setCameraPose()`, which marks a change only when the pose moved. The raster projection uses that camera's fov meanwhile (`rasterVerticalFov()`), so the viewport shows what the render will frame. The camera's widgets and gizmo are disabled during the edit; deleting the camera or loading a scene ends it. Stopping leaves the viewport where it is.

**Unlimited accumulation.** `RenderSettings::unlimitedSamples` (a checkbox next to the max-samples slider; `GpuPathTracer::maxSamples() == 0`): the render never stops by itself, the progress bar becomes indeterminate with the count as its label.

**Restart on change** (`RenderSettings::restartOnChange`, default on). §2.3's snapshot model is now one of two modes. `RaytraceRenderer::update()` builds a `RenderKey` every frame - the render camera's `RTCameraSnapshot`, the settings, the editor revision, the environment map path and intensity, the resolution - and restarts the render when it differs from the running (or finished) render's key. So moving the render camera (gizmo, widgets, or first-person flying), editing a sphere, changing a setting or the environment restarts from scratch; with unlimited samples the output window is a live view. Two deliberate exceptions: the camera's exposure is display-only (applied by the tonemap every frame, live, never a restart), and Stop means stop - no automatic restart until the next Render click. Restarting is cheap: `GpuPathTracer::start()` only waits for the device when the pool size changes; the previous render's sphere buffer is retired through the deletion queue of the frame slot still using it.

**Scene file version 4**: `"cameras": [{name, position, orientation (x,y,z,w), fov, aperture, focusDistance, exposure}]` and `"renderCamera": index`; the `"render"` block loses the lens and exposure and gains `unlimitedSamples` and `restartOnChange`. A version 1–3 file gets one default camera (`render_camera` at the free camera's start pose), seeded with the lens settings its `"render"` block had. `newScene()` creates the same default camera.

**Verified** (scripted hook, validation layers on, 0 messages, then hooks stripped): a fixed-seed 64-sample render from the default scene camera is **bit-identical** to §9.3's render from the free camera at the same pose (230,400 of 230,400 floats); a camera move, a sphere edit and a setting change each restarted the render (sample count back to 1 the next frame); unlimited mode reached 119 samples in 120 frames and was still running; after Stop a camera move did not restart; first-person edit put the viewport at the camera and, after flying, the camera's forward matched the viewport's yaw/pitch exactly; the gizmo's matrix → quaternion round trip is exact; a version-4 file with two cameras saved and reloaded with the second camera's pose, lens, exposure and the render-camera choice intact; a version-2 file loaded with one default camera.

**Not verified - the user's next test pass**: the gizmo lining up with spheres and cameras while flying; the camera frustum overlays; rotating a camera with the gizmo's rings; first-person edit from the panel (RMB look / WASD moving the camera and the render restarting live); the unlimited-samples bar; that exposure changes do not restart while everything else does; the camera combo; a scene with a deleted render camera falling back to the first one.

### 9.8 Second test pass (2026-09-15)

Tests 2–7, 9 and 10 of §9.7 passed. Two fixes followed.

**Gizmo and frustum offset while panning - a resize bug, not a lag.** The camera-update reordering of §9.7 was correct but not the cause: instrumented, the overlay and the raster pass read the same camera position every frame. The real cause was in the window-size handling inherited from the vkguide port, invisible at the exact 1700×900 start size and exposed the moment the window is any other size (on a Retina display the swapchain is the framebuffer, 2× the window in points, while the draw image was fixed at 1700×900): `drawGeometry()` set its per-pipeline viewport and scissor from `m_windowExtent` while the first viewport, the blit and `m_drawExtent` used the draw image's size, so after a resize the geometry was clipped to the window's width inside a 1700-wide image and then stretched to the swapchain with the wrong aspect - a mismatch against the (undistorted) ImGui overlays that grows towards the screen edges, which is where objects go while panning. Fixed by (a) using `m_drawExtent` for every raster viewport and scissor and (b) recreating the draw and depth images at the window's size in points on every resize (`createDrawImages()`/`destroyDrawImages()`, the "Scene Mirror" registration re-done, its visibility kept), so `m_drawExtent`, the viewport, the blit and the swapchain always share one aspect. Verified by reading back the presented swapchain image at a 1400×900 window in two consecutive frames while moving backwards: the raster fills the window and the gizmo sits on the sphere and the frustum on its camera in both. The swapchain now carries `TRANSFER_SRC` usage so such screenshots are possible.

**Restart only while running.** Test 8 exposed a semantics disagreement: edits after a render had *finished* started a new render. The user wants a finished or stopped render to be a snapshot; only a running render restarts on change. `RaytraceRenderer::update()` now restarts only while `m_gpu.isRunning()`, and the `m_stoppedByUser` flag is gone (Stop simply leaves a non-running render). §9.7's description of restart-on-change is superseded on this point.
